...
export const createIntro = async (req, res) => {
  try {
    ...
    if (!name || !description || !techStack) {
      return res
        .status(400)
        .json({ error: "Name, description, and tech stack are required." });
    }

    // const imageUpload = await UploadImage(image, "portfolio")
    // const fileUpload = await UploadImage(file, "portfolio")
    const [imageUpload, fileUpload] = await Promise.all([
      UploadImage(image, "portfolio"),
      UploadImage(file, "portfolio"),
    ]);

    if (!imageUpload || !fileUpload) {
      return res.status(500).json({ error: "Failed to upload image or file." });
    }
    ...
    res
      .status(201)
      .json({ data: newIntro, message: "Intro created successfully!" });
  } catch (error) {
...
export const updateIntro = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ data: updatedIntro, message: "Intro updated successfully!" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
